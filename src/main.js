...
(function () {
	...
	function getUrl(path) {
		if (path.startsWith("content://com.termux.documents/tree")) {
			...
			return androidPath;
		} else if (path.startsWith("file:///data/user/0/com.foxdebug.acode")) {
			let terminalAcode = path.replace(
				"file:///data/user/0/com.foxdebug.acode/files/alpine/home",
				"$HOME",
			);
			//console.log(terminalAcode);
			return terminalAcode;
		} else {
			return false;
		}
	}
	...
	class CodeRunner {
		...
		#projectCommands = [
			{
				...
				handler: (file, { contextMenu }) => (contextMenu ? file.runFile() : file.run()),
			},
		];
		...
		async init($page, cacheFile, cacheFileUrl, firstInit) {
			...
			if (!(await fs(this.#commandsUrl).exists())) {
				...
				if ((data = this.settings.commands)) {
					...
					await fs(this.#commandsUrl).writeFile(
						await fs(PLUGIN_DIR, plugin.id, "default_commands.json").readFile("utf-8"),
						"utf-8",
					);
				}
			}
			...
			this.#commands = JSON.parse((await fs(this.#commandsUrl).readFile("utf-8")) || "[]");
			...
			this.#projectCommands.push({
				...
				async handler(file, { execute }) {
					...
					if (file.name == "package.json") {
						...
						let fs = await acode.fsOperation(Url.join(folder.url, "package.json"));
						if (await fs.exists()) {
		...
		async runCode(contextMenu) {
			...
			if (!contextMenu && (setDefault = handlers.find((h) => h.default))) {
				handler = setDefault;
			} else if (handlers.length > 1 || (handlers.length == 1 && handlers[0].project)) {
				handlers.push({
		...
		async #generateCommand(command, file, changeDir) {
			...
			changeDir = changeDir !== undefined ? changeDir : this.settings.changeDirectory;
		...
		async execute(command) {
			...
			if (this.settings.runLibrary == "AcodeX" && acodex) {
		...
		get settings() {
			...
			if (!value) {
				value = appSettings.value["acode.coderunner.plugin"] = this.initialSettings();
				appSettings.update();
			}
			return value;
		}
